...
class ToolRunner:
    ...
    def build_characters_delta(self, chapter_text: str, base_characters: str) -> Path:
        system_prompt = self._read_prompt("memory_characters_delta.txt")
        user_prompt = f"【已有角色设定】\n{base_characters}\n\n【本章正文】\n{chapter_text}"
        content = self._call(system_prompt, user_prompt, temperature=0.2)
        out_path = self.memory_dir / "updates" / "characters_delta.json"
        self._write(out_path, content)
        self._log("build_characters_delta", {"chapter_len": len(chapter_text)}, {"path": str(out_path)})
        return out_path

    def build_world_rules_delta(self, chapter_text: str, base_world_rules: str) -> Path:
        system_prompt = self._read_prompt("memory_world_rules_delta.txt")
        user_prompt = f"【已有世界观规则】\n{base_world_rules}\n\n【本章正文】\n{chapter_text}"
        content = self._call(system_prompt, user_prompt, temperature=0.2)
        out_path = self.memory_dir / "updates" / "world_rules_delta.md"
        self._write(out_path, content)
        self._log("build_world_rules_delta", {"chapter_len": len(chapter_text)}, {"path": str(out_path)})
        return out_path

    def build_story_so_far_delta(self, chapter_text: str, base_story_so_far: str) -> Path:
        system_prompt = self._read_prompt("memory_story_so_far_delta.txt")
        user_prompt = f"【已有剧情摘要】\n{base_story_so_far}\n\n【本章正文】\n{chapter_text}"
        content = self._call(system_prompt, user_prompt, temperature=0.2)
        out_path = self.memory_dir / "updates" / "story_so_far_delta.md"
        self._write(out_path, content)
        self._log("build_story_so_far_delta", {"chapter_len": len(chapter_text)}, {"path": str(out_path)})
        return out_path

    def save_chapter_draft(self, text: str, chapter_id: str) -> None:
        self._log("save_chapter_draft", {"chapter_id": chapter_id}, {"note": "skipped_write"})
